...
<div class="section">
	<!-- container -->
	<div class="btn_group">
	<a class="a_btn" href="meeting_update.do?meeting_num=${meeting.meeting_num}&meeting_file=${meeting.meeting_file}">
	<button id="update_btn" type="button" class="btn btn-warning">수정하기</button>
	</a>
	<a class="a_btn" href="meeting_delete.do?meeting_num=${meeting.meeting_num}">
		...
		<div class="row">
			<div class="col-md-56">
				<div class="section-title">
					...
					<input type="hidden" class="mentor_id" value="${mentor_id}"/>
					<input type="hidden" class="count_num" value="${count_num}"/>
					<input type="hidden" class="memberCheck" value="${memberCheck}"/>
					<input type="hidden" class="usercount" value="${count_num}"/>
					<!-- body -->
					<div class="meeting_form">
							<div class="meeting_img">
								<img style="border-radius:10px; width: 100%; height: 300px;" alt="" src="/sms/temp/${meeting.meeting_img_name}" />
							</div>
							<br />
							<br />
							<div id="title">
							  	<c:set var="num" value="${meeting.meeting_recruitment}" /><!-- 미팅  명수 -->
							  	<c:set var="n" value="${count_num}" /> <!-- 미팅등록인원 -->
							  <c:choose>
       								<c:when test="${n >= num}">
           										<p class="end text-center" style="font-size:xx-large; font-weight: bold; ">마감되었습니다.</p>
       								</c:when>
       								<c:otherwise>
							...
							<a class="meeting_cancel" href="meeting_cancel.do?user_Id=${user_Id}&meeting_num=${meeting.meeting_num}"><button id="meeting_cancel" type="button" 
							class="btn btn-danger"><i class="fa fa-times-circle" aria-hidden="true"></i> 참여취소하기</button></a>							
							<a class="bt_1" href="list.do"><button id="bt_1" type="button" class="btn"  style="background-color:#9F81F7; bacfont-weight: bold;"><i class="fa fa-sign-out" aria-hidden="true"></i> 이전페이지</button></a>
							</div>
					</div>
				</div>
			</div>
